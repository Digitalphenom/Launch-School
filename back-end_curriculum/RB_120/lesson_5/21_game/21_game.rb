...
module Formatable
  ...
  def outline
    puts PROMPT["win_outline"]
  end

  def seperation_line
    puts PROMPT["seperation_line"]
  end
end
...
class Game
  ...
  def cards_screen
    puts PROMPT["dealer_cards"]
    display_hidden
    seperation_line
    puts sprintf(PROMPT["running_total"]["cards"], player.name)
    puts "#{display_all_cards(player)}"
    puts sprintf(PROMPT["running_total"]["total"], player.card_total)
    new_line
  end
  ...
  def display_all_cards(participant)
    participant.hit.each { |card| puts card}
    nil
  end
  ...
  def evaluate_cards
    player_total = player.card_total
    dealer_total = dealer.card_total
    return winner(player) if dealer.busted? || player_total > dealer_total
    return tie_game if player_total == dealer_total
    winner(dealer)
  end
  ...
  def dealer_turn
    ...
    puts PROMPT["dealer_turn"]
    until dealer.card_total >= 17
      hit_and_add_total(dealer)
      check_for_eleven(dealer)
      puts count.odd? ? PROMPT["dealer_hits"] : PROMPT["dealer_hits_again"]
      display_hidden
      count += 1
    end
    puts PROMPT["dealer_stays"] unless dealer.busted?
  end
